...
function pull() {
  return act(() => api(`/api/player/${model.playerId}/pull`, { method: "POST" }));
}
...
function render() {
  ...
  document.querySelector("#pullBtn").disabled = busy || state.coins < 100;
  ...
  document.querySelector("#districts").innerHTML = data.districts.map((district) => {
    const locked = busy || state.coins < district.cost || derived.power < district.power;
    const need = district.power > derived.power ? `Need ${district.power} power` : district.cost > state.coins ? `Need ${district.cost} coins` : "Ready";
    return `
      <div class="district">
        <div>
          <h3>${district.name}</h3>
